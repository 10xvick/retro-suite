...
async function testMode3ReactUI() {
  console.log("\n==========================================================================");
  console.log(" REACT UI CANVAS AUTOMATION FOR BASIC MODE 3");
  ...
  console.log('Opened Web UI in non-headless Chrome window.');
  ...
  console.log(`Loading bios (${biosBytes.length} bytes) and suite.gba (${romBytes.length} bytes) directly into React main canvas UI...`);
  ...
  const result = await page.evaluate(async (biosArr, romArr) => {
    ...
    // Attach GBA rendering output directly to React main canvas element in the UI!
    ...
    // 1. Initial Boot: 60 frames to reach Main Menu, rendered directly on React screen
    for (let f = 0; f < 60; f++) {
      gba.runFrame();
    ...
    // 4. Press A (X in UI) to open/start Live Actual Screen rendering!
  ...
  console.log("\n--- REACT UI CANVAS BASIC MODE 3 RESULTS ---");
  console.log(`Subtest: ${result.subtest}`);
  console.log(`Actual DISPCNT   : ${result.actualDispcnt}`);
  console.log(`Expected DISPCNT : ${result.expectedDispcnt}`);
  ...
  const actualPath = savePng(result.actualDataUrl, `react_ui_mode3_actual.png`);
  const expectedPath = savePng(result.expectedDataUrl, `react_ui_mode3_expected.png`);
  const diffPath = savePng(result.diffDataUrl, `react_ui_mode3_diff.png`);
  ...
  await browser.close();
  console.log("React UI canvas automation complete!");
}

testMode3ReactUI().catch(err => {
  console.error("React UI test error:", err);
  process.exit(1);
});
