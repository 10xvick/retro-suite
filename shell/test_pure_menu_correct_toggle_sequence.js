...
async function testPureMenuCorrectToggleSequence() {
  console.log("\n==========================================================================");
  console.log(" CORRECT TOGGLE SEQUENCE FOR PURE MENU AUTOMATION (ZERO CPU REGISTER HACKS)");
  ...
  const results = await page.evaluate(async (biosArr, romArr) => {
    ...
    for (const sub of subtests) {
      // 100% PURE MENU AUTOMATION — ZERO CPU REGISTER HACKS!
      const gba = new GBA();
      gba.loadBios(bios);
      gba.loadCart(cart);
      gba.reset();
      ...
      const press = (k, holdFrames = 30, releaseFrames = 30) => {
      ...
      // 4. Press LEFT to switch to LIVE ACTUAL VIEW (<Actual> badge)!
      press(keyLeftPressed);
      for (let f = 0; f < 20; f++) gba.runFrame();

      // 5. For Subtests 4 & 5, press A ON THE LIVE VIEW to toggle BG0 layer ON!
      ...
      // 6. Press RIGHT to switch to GOLDEN EXPECTED VIEW (<Expected> badge)!
  ...
  console.log("\n==========================================================================");
  console.log(" CORRECT TOGGLE SEQUENCE AUDIT SUMMARY FOR ALL 7 SUBTESTS");
  console.log("==========================================================================");
  ...
  for (const r of results) {
    ...
    const actualPath = savePng(r.actualDataUrl, `seq_sub${r.id.toString().padStart(2, '0')}_actual.png`);
    const expectedPath = savePng(r.expectedDataUrl, `seq_sub${r.id.toString().padStart(2, '0')}_expected.png`);
    const diffPath = savePng(r.diffDataUrl, `seq_sub${r.id.toString().padStart(2, '0')}_diff.png`);
  ...
  console.log("\nKeeping browser window open for 20 seconds so you can see it...");
  await new Promise(r => setTimeout(r, 20000));
  await browser.close();
}

testPureMenuCorrectToggleSequence().catch(err => {
