...
async function testPureMenuHeldKeys() {
  console.log("\n==========================================================================");
  console.log(" PURE MENU AUTOMATION WITH HELD KEYINPUT (NO CPU REGISTER HACKS)");
  ...
  const results = await page.evaluate(async (biosArr, romArr) => {
    ...
    for (const sub of subtests) {
      // 100% PURE MENU EXECUTION — ZERO CPU REGISTER MANIPULATION!
      const gba = new GBA();
      gba.loadBios(bios);
      gba.loadCart(cart);
      gba.reset();
      ...
      // 4. HOLD LEFT D-PAD ACTIVELY DURING RENDERING FOR TRUE ACTUAL SCREEN (<Actual> badge)!
      ...
      // 5. HOLD RIGHT D-PAD ACTIVELY DURING RENDERING FOR TRUE EXPECTED SCREEN (<Expected> badge)!
  ...
  console.log("\n==========================================================================");
  console.log(" PURE MENU EXECUTION WITH HELD KEYINPUT SUMMARY FOR ALL 7 SUBTESTS");
  console.log("==========================================================================");
  ...
  for (const r of results) {
    ...
    const actualPath = savePng(r.actualDataUrl, `held_sub${r.id.toString().padStart(2, '0')}_actual.png`);
    const expectedPath = savePng(r.expectedDataUrl, `held_sub${r.id.toString().padStart(2, '0')}_expected.png`);
    const diffPath = savePng(r.diffDataUrl, `held_sub${r.id.toString().padStart(2, '0')}_diff.png`);
  ...
  console.log(`\nAUDIT COMPLETE: ${totalPassed} / ${results.length} Subtests passed under pure held-key menu execution.`);
  await browser.close();
}

testPureMenuHeldKeys().catch(err => {
  console.error("Held key audit error:", err);
  process.exit(1);
});
