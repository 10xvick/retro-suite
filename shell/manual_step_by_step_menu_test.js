...
async function runManualStepByStepTest() {
  console.log("\n==========================================================================");
  console.log(" MANUAL STEP-BY-STEP MENU TRAVERSAL (GOLDEN REFERENCE MATCH)");
  ...
  console.log(`Loaded bios (${biosBytes.length} bytes) and suite.gba (${romBytes.length} bytes). Traversing menus step-by-step for all 7 subtests...`);
  ...
  console.log("\n--- STEP-BY-STEP MANUAL MENU PARITY REPORT ---");
  for (const r of results) {
    ...
    const actualPath = savePng(r.actualDataUrl, `manual_step_sub${r.id.toString().padStart(2, '0')}_actual.png`);
    const expectedPath = savePng(r.expectedDataUrl, `manual_step_sub${r.id.toString().padStart(2, '0')}_expected.png`);
    const diffPath = savePng(r.diffDataUrl, `manual_step_sub${r.id.toString().padStart(2, '0')}_diff.png`);
  ...
  await browser.close();
  console.log("\nManual step-by-step menu traversal complete!");
}

runManualStepByStepTest().catch(err => {
  console.error("Manual step test error:", err);
  process.exit(1);
});
