...
async function runBrowserAutomationSuite() {
  console.log("\n==========================================================================");
  console.log(" FRESH BROWSER AUTOMATION SUITE: GBA CATEGORY 13 VIDEO TESTS");
  console.log(" URL: http://localhost:5006/emulator/retro-station/");
  console.log(" Screenshot Destination: " + screenshotsDir);
  console.log("==========================================================================\n");
  ...
  console.log('Opened Retro Station GBA Emulator in non-headless Chrome window.');
  ...
  console.log(`Loaded bios (${biosBytes.length} bytes) and suite.gba (${romBytes.length} bytes). Executing manual menu step automation...`);
  ...
  const results = await page.evaluate(async (biosArr, romArr) => {
    ...
    let overlayCanvas = document.getElementById('automation-overlay-canvas');
    if (!overlayCanvas) {
      overlayCanvas = document.createElement('canvas');
      overlayCanvas.id = 'automation-overlay-canvas';
      ...
      overlayCanvas.style.boxShadow = '0 0 20px rgba(0,255,204,0.6)';
  ...
  console.log("\n--- FRESH BROWSER AUTOMATION PARITY REPORT ---");
  for (const r of results) {
    ...
    const actualPath = savePng(r.actualDataUrl, `fresh_auto_sub${r.id.toString().padStart(2, '0')}_actual.png`);
    const expectedPath = savePng(r.expectedDataUrl, `fresh_auto_sub${r.id.toString().padStart(2, '0')}_expected.png`);
    const diffPath = savePng(r.diffDataUrl, `fresh_auto_sub${r.id.toString().padStart(2, '0')}_diff.png`);
  ...
  await browser.close();
  console.log("\nFresh browser automation suite complete!");
}

runBrowserAutomationSuite().catch(err => {
  console.error("Browser automation suite error:", err);
  process.exit(1);
});
