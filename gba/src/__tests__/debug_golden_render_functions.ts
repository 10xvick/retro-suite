...
console.log("=== DEBUGGING GOLDEN RENDER FUNCTIONS ===");
...
for (const sub of subtests) {
  ...
  const pngPath = path.join(artifactDir, `golden_fn_subtest_${sub.id}.png`);
  saveBufferAsPng(240, 160, fb, pngPath);

  console.log(`Subtest #${sub.id} ("${sub.name}") Golden FN output: DISPCNT=0x${gba.ppu.dispcnt.toString(16)} -> ${pngPath}`);
}
