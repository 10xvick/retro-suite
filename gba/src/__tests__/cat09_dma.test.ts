...
describe('GBA Hardware Test Category: 09 DMA tests', () => {
  let gba: GBA;
  let cart: Uint8Array;
  ...
  it('09 DMA tests', () => {
    ...
    gba.cpu.r[0] = 0x8041b60;
    gba.cpu.r[1] = 0x03007b08;
    gba.cpu.r[14] = 0x08000100;
    gba.cpu.cpsr = (gba.cpu.cpsr & ~0x20) | ((0x80063a1 & 1) ? 0x20 : 0);
    gba.cpu.r[15] = 0x80063a1 & ~1;

    for (let f = 0; f < 300; f++) gba.cpu.step();

    const totalSubtests = gba.mem.read32(0x8041b60 + 12);
    let failures: string[] = [];
    ...
    for (let i = 0; i < totalSubtests; i++) {
      ...
      if (actual !== expected && status !== 0) {
        ...
        failures.push(
          `\n  ❌ [FAIL] Category: "09 DMA tests" | Test: "${testName}"` +
          `\n     • Expected:           0x${expHex}` +
          `\n     • Actual:             0x${actHex}` +
          `\n     • Bit Diff (XOR):     0x${diffHex}` +
          `\n     • ROM Entry Routine:  0x80063a1 (Desc: 0x8041b60)` +
    ...
    console.log(`\n==========================================================================`);
    console.log(` CATEGORY: 09 DMA tests`);
    ...
    expect(failures.length, `Failures detected in 09 DMA tests:
${failures.join('\n')}`).toBe(0);
  });
});
