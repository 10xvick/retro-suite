...
describe('GBA Hardware Test Category: 07 Multiply long tests', () => {
  let gba: GBA;
  let cart: Uint8Array;
  ...
  it('07 Multiply long tests', () => {
    gba.reset();
    gba.directBoot();

    gba.cpu.r[0] = 0x8041e78;
    gba.cpu.r[1] = 0x03007b08;
    gba.cpu.r[14] = 0x08000100;
    gba.cpu.cpsr = (gba.cpu.cpsr & ~0x20) | ((0x8007259 & 1) ? 0x20 : 0);
    gba.cpu.r[15] = 0x8007259 & ~1;

    for (let f = 0; f < 300; f++) gba.cpu.step();

    const totalSubtests = gba.mem.read32(0x8041e78 + 12);
    let failures: string[] = [];
    ...
    for (let i = 0; i < totalSubtests; i++) {
      ...
      if (actual !== expected && status !== 0) {
        ...
        failures.push(
          `\n  ❌ [FAIL] Category: "07 Multiply long tests" | Test: "${testName}"` +
          `\n     • Expected:           0x${expHex}` +
          `\n     • Actual:             0x${actHex}` +
          `\n     • Bit Diff (XOR):     0x${diffHex}` +
          `\n     • ROM Entry Routine:  0x8007259 (Desc: 0x8041e78)` +
    ...
    console.log(`\n==========================================================================`);
    console.log(` CATEGORY: 07 Multiply long tests`);
    ...
    expect(failures.length, `Failures detected in 07 Multiply long tests:
${failures.join('\n')}`).toBe(0);
  });
});
