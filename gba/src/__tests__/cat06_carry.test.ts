...
describe('GBA Hardware Test Category: 06 Carry tests', () => {
  let gba: GBA;
  let cart: Uint8Array;
  ...
  it('06 Carry tests', () => {
    ...
    for (let i = 0; i < totalSubtests; i++) {
      ...
      if (actual !== expected && status !== 0) {
        ...
        failures.push(
          `\n  ❌ [FAIL] Category: "06 Carry tests" | Test: "${testName}"` +
          `\n     • Expected:           0x${expHex}` +
          `\n     • Actual:             0x${actHex}` +
    ...
    console.log(`\n==========================================================================`);
    console.log(` CATEGORY: 06 Carry tests`);
    ...
    expect(failures.length, `Failures detected in 06 Carry tests:
${failures.join('\n')}`).toBe(0);
  });
});
